...
// #define SERVERIP "192.168.1.108"  // zuhause
 #define SERVERIP "169.254.93.65" // technikum
...
Void tcpHandler(UArg arg0, UArg arg1)
{
    ...
    Task_Handle taskHandle_recv;
    Task_Params taskParams_recv;
    Error_Block eb_recv;
    Error_init(&eb_recv);
    Task_Params_init(&taskParams_recv);

        taskParams_recv.stackSize = 2048;
        taskParams_recv.priority = 1;
        taskParams_recv.arg0 = (UArg) socket;
        taskHandle_recv = Task_create((Task_FuncPtr)recvHandler, &taskParams_recv, &eb_recv);
        if (taskHandle_recv == NULL) {
            System_printf("main: Failed to create recvHandler Task\n");
        } else {
